...
|타입 | 설명 |  
|--|--|  
| 공개 테스트 | 대상: <br/> - 실제 유저가 직접 참여 가능 <br/>- 등록한 테스터 그룹 참여 가능<br/>특징:<br/> - 실제 유저 즉 **누구나 Google Play를 통해** 출시 전 버전을 테스트 참여를 가능하게 함<br/>- 앱 등록시 스토어 **심사를 하게 됨**|  
|비공개 테스트| 대상: <br/>- 등록한 테스터 그룹 참여 가능 <br/>특징:<br/>- 트랙을 만들어 **자체 지정 테스터 그룹**에 테스트 참여를 가능하게 함<br/>- 앱 등록시 스토어 **심사를 하게 됨**|  
|내부 테스트| 대상: <br/>- **등록한 테스터 그룹** 참여 가능 <br/>특징:<br/>- 앱 등록시 스토어 **심사를 하지 않음**|
...
프러덕션이 버전과 국가/지역 설정을 동기화 할수 있고 자체로 지정 할 수 있음.
...
|메뉴| 설명  |  
|--|--|  
|테스터  | 참여인원수를 무제한 혹은 제한 할수 있음, 제한 할 경우 최소 1,000명이상 |  
|의견 URL 또는 이메일 주소| 의견을 받은 사이트 주소나 이메일 주소 입력|  
| Android에서 참여| 스토어에서 바로 참여 할 수 있는 링크 복사 |  
| 웹에서 참여| 웹에서 참여 확인 할 수 있는 링크 복사 |

- Android에서 참여 할때 **반드시 요청한 이메일이 폰의 Google Playstore 에 로그인 되어 있어야** 테스트모드로 접근 가능 .
- 웹에서 참여 할때 방문한 페이지에 구글계정은 **반드시 테스트 요청한 이메일로 로그인한 상태**여야 **참여 수락**을 하거나 **참여 취소**할 수 있음
...
|메뉴| 설명  |  
|--|--|  
|테스터  | - 테스터 이메일 목록으로 관리 중인 리스트 선택([목록 만들기 자세히 알아보기](#4.4-테스터-이메일-목록-만들기))<br/>- 내가 지정한 Google 그룹스에 속한 사용자만 테스트에 참여<br/>|  
|의견 URL 또는 이메일 주소| 의견을 받은 사이트 주소나 이메일 주소 입력|  
| Android에서 참여| 스토어에서 바로 참여 할 수 있는 링크 복사 |  
| 웹에서 참여| 웹에서 참여 확인 할 수 있는 링크 복사 |
...
|메뉴| 설명  |  
|--|--|  
|테스터  |  테스터 이메일 목록으로 관리 중인 리스트 선택([목록 만들기 자세히 알아보기](#4.4-테스터-이메일-목록-만들기))|  
|의견 URL 또는 이메일 주소| 의견을 받은 사이트 주소나 이메일 주소 입력|  
| 웹에서 참여| 웹에서 참여 확인 할 수 있는 링크 복사 |


-  **반드시 요청한 이메일이 폰의 Google Playstore 에 로그인 되어 있어야** 테스트모드로 접근 가능 .
- 웹에서 참여 할때 방문한 페이지에 구글계정은 **반드시 테스트 요청한 이메일로 로그인한 상태**여야 **참여 수락**을 하거나 **참여 취소**할 수 있음
...
| 이메일 주소 추가 | **폰의 PlayStore 나 브라우저에 로그인 되어 있는 이메일주소**를 제공해야 함,그래야만  참여요청 할때 Android 혹은 웹으로 초대 할때 해당 이메일로 로그인한 상태에서만 수락이나 거절 등 관리를 할 수 있음|
...
출시 노트를 적은후에 **저장&버전 검토**를 눌러 다음단계로 이동
...
배포타입에 따라 **출시율** 설정이 있고 없으면 안해도 됨.
**출시율**은 이미 설치한 유저수 대비 퍼센테지만큼의 유저한테 출시를 할지 정함.
...
(내부 테스트 외에 모두 심사가 필요하기에 다소 시간이 걸림. 심사 시간은 **심사정책에 따라 30분~7일까지 차이**가 있을수 있음)
